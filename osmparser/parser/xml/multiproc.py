...
from osmparser.parser.util import setproctitle
...
class XMLChunker(object):
    ...
    def _skip_header(self):
        for line in self.stream:
            if line.lstrip().startswith('<node '.encode()):
    ...
    def _new_xml_outstream(self):
        ...
        stream.seek(0)
        stream.write("<osm xmlns:xapi='http://www.informationfreeway.org/xapi/0.6'>".encode())
        return stream

    def _finished_xml_outstream(self, last_line, stream):
        if '</osm'.encode() not in last_line:
            stream.write('</osm>\n'.encode())
    ...
    def read(self, mmaps_queue, coords_callback=None):
        ...
        for line in self.stream:
            if coords_callback:
                coord_node_match = coord_node_re_match(line.decode())
                if coord_node_match:
                ...
                xml_nodes.write(line)
            if split:
                if (line.lstrip().startswith('</'.encode())
