...
class ShootingStarTest():
    ...
    def createTable(self):
        self.cursor.execute('CREATE TABLE edgeTableShootingStar (gid INTEGER PRIMARY KEY, id INTEGER, source INTEGER, target INTEGER, cost REAL, reverseCost REAL, x1 REAL, y1 REAL, x2 REAL, y2 REAL, toCost REAL, rule TEXT)')
    ...
    def addToTable(self, source, target, cost, reverseCost, x1, y1, x2, y2, toCost, rule):
        self.cursor.execute('INSERT INTO edgeTableShootingStar VALUES( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (self.gid, self.edgeId, source, target, cost, reverseCost, x1, y1, x2, y2, toCost, rule))
        self.gid=self.gid+1
        self.edgeId=self.edgeId+1
    
    def testTable(self):
        self.cursor.execute('SELECT * FROM edgeTableShootingStar')
